...
class TrackedPoints:
    ...
    Attributes:
        ...
        displacements: N x 2 array of vectors (dx, dy) in pixels.
...
class OpticalFeatureTracker:
    ...
    def track(self, frame_bgr_or_gray: np.ndarray) -> TrackedPoints:
        """Process incoming frame and compute optical flow tracking from previous frame.

        Args:
            frame_bgr_or_gray: Incoming image frame.

        Returns:
            TrackedPoints object with trajectories, displacements, and quality scores.
        """
        ...
        if self.prev_gray is None or self.tracked_pts is None:
            ...
            self.tracked_pts = self.detect_features(curr_gray)
            self.initial_pts_count = len(self.tracked_pts) if self.tracked_pts is not None else 0

            empty_pts = np.empty((0, 2), dtype=np.float32)
            quality = "GOOD" if self.initial_pts_count >= self.config.min_tracked_points else "LOST"
            return TrackedPoints(
                prev_pts=empty_pts,
                curr_pts=empty_pts,
                displacements=empty_pts,
                status=np.empty(0, dtype=bool),
                valid_count=self.initial_pts_count,
                tracking_quality=quality,
                quality_score=1.0 if self.initial_pts_count >= self.config.min_tracked_points else 0.0,
                redetected=True,
            )
        ...
        if new_pts is not None and st is not None:
            ...
            disp = np.empty((0, 2), dtype=np.float32)
        ...
        redetected = False
        if valid_count < self.config.min_tracked_points:
            new_features = self.detect_features(curr_gray)
            if new_features is not None and len(new_features) >= self.config.min_tracked_points:
                self.tracked_pts = new_features
                self.initial_pts_count = len(new_features)
                valid_count = len(new_features)
                score = 1.0
                quality = "GOOD"
                redetected = True
            else:
    ...
    def reset(self) -> None:
        """Reset internal history and force re-detection on next frame."""
        self.prev_gray = None
